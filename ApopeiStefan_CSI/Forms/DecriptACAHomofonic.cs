...
{
    public partial class DecriptACAHomofonic : Form
    {
        public string TextDeCriptat { get; set; }
        public string Cheia{ get; set; }
        public DecriptACAHomofonic()
        {
            InitializeComponent();
        }
        ...
        private void OnClickDecriptare(object sender, EventArgs e)
        {
            try
            {
                ...
                for(int i=0;i<TextDeCriptat.Length;i+=2)
                {
                    for (int j = 0; j< alfabet.Length; j++)
                    {
                        for (int x = 0; x < 4; x++)
                        {
                            if (matrix[x, j].ToString().Contains((TextDeCriptat[i].ToString() + TextDeCriptat[i + 1].ToString())))
                                textBoxTextDecriptat.Text = textBoxTextDecriptat.Text + alfabet[j];
                        }
                    }
                }
            }
